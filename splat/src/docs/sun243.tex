...
To lookup the coordinates of an object just type in an identifier and either
press \hitext{<Return>} or the \labelitem{Lookup} button. Alternatively if you
known the coordinates (FK5/J2000) of the region of sky you can just enter the
coordinates in the \labelitem{RA} and \labelitem{Dec} fields. The 
\labelitem{Radius} value is a number of arcminutes to search about the
position. Once you have identified the region to search press the 
\labelitem{Go} button to contact the current list of SSAP servers. To make a
query it may be necessary for you to tell \SPLAT\ about your local web-proxy
server (this is the same as that you use in your Internet browser), select the
\submenuitem{Options}{Configure proxy...} item to do this.

The list of servers available is shown in the \labelitem{Servers} menu. To 
not query a server de-select it from this list (note that in the future the
list of servers itself will be determined by a query to a VO registry).
...
\submenuitem{Options}{Match coordinates and/or fluxes} item in the plot window.

\subheading{Status of implementation}

SSAP is a protocol being developed by the IVOA 
(\htmladdnormallinkfoot{International Virtual Observatory Alliance}
{http://www.ivoa.net/}) and is not yet a full recommendation so the 
services offered here are just a flavour of what might become available. 

Currently responses from SSAP servers may only return spectra in the simple
formats understood by \SPLAT\ (these are generally FITS of various flavours)
and the SSAP query is limited to just a cone region on the sky (additional
meta-data like XXXpassbandXXX are not allowed). The servers themselves are
currently hard-coded into \SPLAT, but this situation will be resolved in the
next release which will feature a system for query VO registries for SSAP
services.

The matching of spectral coordinates and fluxes is performed by the Starlink
AST library (\xref{SUN/211}{sun211}). This works using a full implementation
of spectral coordinates from FITS paper III and by interpreting the units
strings (this does not use the dimensional analysis-only approach).
...
Sun (heliocentric) has a recessional velocity of 5300km/s.
...
This window allows you to inspect and set the units of the data values of your
spectra. Currently the only use made of these values in \SPLAT\ is when
displaying more than one spectrum in a plot window. In that case you can
...
frequency and per unit wavelength systems. For instance all the following
...
\item by collapsing images into a single \hitext{spectrum} by adding together
all values that lie perpendicular to the dispersion axis (so the spectrum runs
along this axis, what a good spectral displaying tool should do),

\item by expanding images and cubes into a spectrum for each line running
along the dispersion axis,

\item by vectorizing the whole data into a single line (created by joining
...
Starlink SPLAT now a tool for the Virtual Observatory and has been 
renamed to \SPLAT.
