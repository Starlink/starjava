...
% Not advertising this yet. Maybe big changes ahead.
...
\section{Tool interoperability using PLASTIC\xlabel{plastic}}
...
If the hub is running when GAIA starts up, \SPLAT\ will connect to it
...
The \menuitem{Interop} menu has various \menuitem{Accept spectra as \ldots}
checkboxes (some of these are set by default and some not).
The ones which are set indicate that \SPLAT\ is listening out for
messages from other applications which want to send data in that format.
If another tool sends data in that format to \SPLAT\ or to all
listening applications, then \SPLAT\ will attempt to load it as
a spectrum and add it to the global list just as if you had 
loaded it in the normal way.

Unfortunately, there is no guarantee that data sent in this way by other
tools actually represent a spectrum, so it is not always wise to 
accept these things - for instance a VOTable being sent by another
tool may represent a source catalogue rather than a spectrum,
in which case attempting to display it as a spectrum wouldn't be very 
useful.  These Load messages will normally arrive through the
PLASTIC system only if you, the user, take some action to send them
(e.g.\ click a ``Send table'' button in a different tool), so you
shouldn't have to worry about receiving them from out of the blue, 
but it is sometimes helpful to filter the ones which arrive
using these checkboxes.
...
\menuitem{Broadcast spectrum as \ldots} or \menuitem{Send spectrum as \ldots}
items.  
The Broadcast actions send the data to all listening tools.
The Send actions send only to a single selected one; you choose it
from the submenu to the right.  Only tools which know what to do
with that kind of data will be listed; if none of the currently-registered
tools is prepared to accept data in that format, the Send options
will be unavailable.
...
\subsection{This release (3.4-1)}

The use of file type \hitext{guessing} has been extended beyond the 
\submenuitem{File}{Browse} dialogue. This can now be selected from the
command-line, or from the \submenuitem{File}{Open} dialogue. Guessing
requires that the spectrum be opened for each candidate type, so is
intrinsically slower than file extension based typing. Remote resources
will, in general, be opened using guessing.

\subsection{Release (3.4)}
