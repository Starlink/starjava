...
AST also provides the flux conversion utilities used by \SPLAT\ which is based
on interpreting the units strings system described in FITS-WCS paper I
\textit{Representation of World Coordinate in FITS}
by Greisen \& Calabretta. This does not use the dimensional analysis-only
approach which cannot deal with dimensionless systems like magnitudes and
conversions from Jy to Jy/sr and Jy to temperature, however, at present
flux systems are restricted to flux per unit frequency and per unit wavelength.
