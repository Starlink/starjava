...
\newcommand{\stardocnumber}    {243.2}
\newcommand{\stardocauthors}   {Peter W. Draper}
\newcommand{\stardocdate}      {03 June 2003}
...
\begin{center}
\begin{quote}
\textit{This is a `beta' release of \SPLAT\ which is now part of the
Starlink Java Collection. The older `classic' version of \SPLAT\ is
still available, but will no longer be updated and will be eventually
retired. To use this new version you need to redefine the
\hitext{SPLAT\_DIR} variable to point at the new \hitext{bin/splat}
directory.}
\end{quote}
\end{center}
...
Display windows can be interactively zoomed and scrolled, centered on
specific wavelengths, provide continuous coordinate readout,
...
drag-and-drop and undoable editing.

The current set of analysis facilities include the fitting of a
polynomial to selected parts of a spectrum, the fitting of Gaussian,
Lorentzian and Voigt profiles to emission and absorption lines and the
filtering of spectra using average, median and line-shape window
functions as well as wavelet denoising.
A database of laboratory line positions is available to aid in
identification.
...
spectra stored in NDF, FITS and TEXT files, as well as NDX format.
...
\submenuitem{File}{Open} dialog. This will produce a main window that
looks something like:

\mainfigure{browser3}

If you've opened all the spectra at once (you can do this using the
\submenuitem{File}{Open} dialog by selecting more than one file --
...
To make the identification of the spectra clearer you can just press
the auto-colour button: \inline{rainbow}. Each press of this
button should generate a new set of colours, so just keep clicking
until you see a combination that you like.
...
\labelitem{Y scale:} controls. To adjust the zoom you
may either select a value from the drop-down lists, or you may type in
a value and press \hitext{<Return>}. Values may be non-integer, but
...
displayed spectra to fit the visible width or height.  This feature
can also be useful if you've resized the plot window.
...
\begin{quote}
 ...
 that are displayed in the currently selected spectra (this time if a
 ...
 already displaying some of the selected spectra the you force it to
...
\begin{quote}
 ...
 \begin{itemize}
  ...
  \item[\inline{openfile}] Create an open file dialog. You can use this
  ...
  \item[\inline{savefile}] Create a save file dialog. Using this you can
  ...
  energy and frequency or even helocentric and topocentric rest frames
  \etc.).
...
\begin{quote}
 ...
 look in the \labelitem{Configure Plot Options} window, which is activated
 by pressing the \inline{config} button.

 The \labelitem{Configure Plot Options} window also offers the ability
 ...
 \begin{itemize}
  ...
  \item Interactive graphics control: if any interactive graphics
...
\begin{quote}
 ...
 The \labelitem{Displaying:} drop down list shows the names and
...
\begin{quote}
 ...
 \begin{itemize}
  ...
  \item[\inline{print}] Print a copy of the display area to a
  ...
  encapsulated but is known to be convertible by the \hitext{pstoespi}
  ...
  \item[\inline{jpeg}] Make a JPEG image version of the display
  ...
  \item[\inline{fitwidth}] Make spectra fit the display area width.
  Use this to quickly return to an unzoomed coordinate state and to
  match the X axis width when you resize the plot window.

  \item[\inline{fitheight}] Make spectra fit the display area height.
  ...
  backgrounds prior to line fitting, or for subtracting backgrounds
  from spectra.

  \item[\inline{fitline}] Show the toolbox for fitting emission and
  ...
  define a polynomial background before using this tool.
 ...
 The \menuitem{Options} menu contains a single item
 \submenuitem{Options}{Match coordinates}. 
 When this is selected the plot will attempt to use any coordinate
 information associated with the displayed spectra to \textit{align}
 ...
 source (\ie\ they will be red or blue shifted if you have
...
This window allows you to display a series of spectra from the global
list, one after the other. It is possible to display these into an
existing plot, that may already be displaying other spectra you'd like
to use for comparison, or you can use a new plot.
...
\begin{quote}
 ...
 dialogs that either offer special functions (linear transformations),
...
\begin{quote}
 ...
 (the top part of this is greyed out as Voigt is a builtin function
 ...
 of the \xref{KAPPA SUN}{sun95}{ap:MathMaps}. A lot of C-like
...
\begin{quote}
 ...
 A simple builtin option of applying a linear transformation is
...
library documentation (\xref{SUN/211}{sun211}{ss:specframes}).
...
%%% Axes window is not described.

Normally to make any configuration changes happen you should press the
\labelitem{Apply} button, unless the \labelitem{Auto-update} option
has been applied.
...
This page allows you to set the physical limits of the axis drawn in
the display area. The default configuration autoscales the display
area to the full limits of all spectra that it is displaying.

If you want to apply limits of your own then, untick the autoscale
checkboxes and enter your values in the lower and upper entry
areas. Remember to press \labelitem{Apply} to apply the limits.
...
This page allows you to configure how the axis number labels are
rendered. As with other pages you can define the font, style, size and
colour used, as well as the separation between the actual axes and the
labels. An accurate rendering of how the labels will be drawn is shown
in the \labelitem{Sample:} label.
...
This page allows you to define how carefully the graphics are drawn in
the plot display area. By default any text is drawn using
anti-aliasing, which makes the graphics less jagged, and spectra are
drawn without anti-aliasing. If you'd also like the spectra to be
anti-aliased tick \labelitem{Everything:}.
...
This additional window is activated by the \labelitem{(Re)Store}
button. It's purpose is to allow you to create a database of
configurations that you'd like to keep (after all if you've spent some
time configuring the look of a plot you don't want to have to type it
all in again when someone asks you to tweak the postscript).
...
\labelitem{Add} button. This creates a new entry with the description
...
The polynomial fitting window allows you to fit a polynomial of
selected degree to various selected ranges of the current
spectrum. You can select to use errors as weights for the fit and can
save the fits on the global list as spectra in their own
right. Fitting a polynomial is a pre-requisite to fitting spectral
lines.
...
allow you to ask for the creation of a spectrum that is the difference
...
\labelitem{Background fit}: To measure a line you must first define a
background spectrum (say using the polynomial fitting tool) or have
spectra that are already background subtracted.

Use this drop-down list to select the appropriate spectrum from the
global list. If you don't want to use a background self-select the
spectrum you're measuring. The default background should be set to the
first polynomial found on the global list.
...
\begin{quote}
 ...
 \begin{itemize}
   ...
   \item \hitext{Equiv}, the equivalent width of the line. This will be
         $0$ if no background fit is given.
...
\begin{itemize}
  ...
  \item \hitext{Wavelet} use various wavelet filters to convert the
        spectrum into multiresolution coefficients, removing a
...
\SPLAT\ can read and write spectra stored in four fundamental formats,
Starlink NDF \& NDX, FITS and simple TEXT. NDFs are stored in
container files that have the extension \hitext{.sdf}. FITS files have
one of the extensions \hitext{.fits} or \hitext{.fit} and TEXT files
\hitext{.txt} or \hitext{.lis}. NDX format spectra, when stored on disk,
will have extension \hitext{.xml}.

Line identifier catalogues currently have one format - TEXT based in
files with extension \hitext{.ids}.

Note that although there are a set of default file extensions you can
select the type that a spectrum is opened as using the 
\submenuitem{File}{Open} dialog in the main window. Currently it isn't
possible to do this from the command-line.

Spectra stored in TEXT files must have the simple format of two or
three columns of data, separated by whitespace (that is spaces or
tabs). Any comments in the file must be indicated by a hash sign,
blank lines are also allowed. The three columns should have the
following values:
\begin{quote}
\begin{verbatim}
...
\end{verbatim}
\end{quote}
The error column being optional.

The format of line identification catalogues is like that of spectral
files, except the second column contains a string that identifies the
line (with no spaces, \ie\ \hitext{H-alpha} not \hitext{H
alpha}). It is also possible to define the system of the coordinate
values in a header section, see the \SPLAT\ builtin catalogues for
examples of this.

NDFs stored within container files can be accessed, but only by
entering the full HDS path on the command-line or in the
\submenuitem{File}{Open} dialog.  FITS extensions may also be open in
this fashion (by appending the extension number in square brackets).
...
%The most useful of the remote control examples are the commands:
...
only make use of \SPLAT\ classes, so don't need \SPLAT\ to be
running. These are:
\begin{itemize}
...
\section{Changes}
\subsection{This release, 2.0}
In this release \SPLAT\ has become part of the Starlink Java
Collection and is now released under the GPL. 

The most significant changes are:
\begin{itemize}
\item The addition of facilities for direct viewing of spectral
      values and power (\ie\ algebraic with undo) modifications of the
      values.
\item The addition of line identifier catalogues (and support for
      importing external versions).
\item The addition of support for spectral coordinate systems. Spectra
      can now have systems that define the coordinate type (wavelength,
      frequency, energy and velocity), along with the units
      (which include, from much larger flexible set, angstroms,
       ...
       automatically and performed operations like change of rest
       frame (say from topocentric to heliocentric, or centre of the
       galaxy or to the source itself) or change of units.
\item The addition of smoothing filters, average, median, Gauss,
