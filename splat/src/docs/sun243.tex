...
{Copyright \copyright\ 2001-2005 Council for the Central Laboratory of the Research Councils \\
 ...
 Copyright \copyright\ 2010-2017 Peter W. Draper \\
 Copyright \copyright\ 2013 German Astrophysical Virtual Observatory (GAVO)}
\stardocnumber      {243.42}
\stardocauthors     {Peter W. Draper, Margarida Castro Neves \& Mark Taylor}
\stardocdate        {1 Apr 2018}
...
\SPLAT\ can also display time series in certain formats, and search for spectral lines
using VO and VAMDC services . This are still  experimental features, and will evolve 
as  the respective IVOA standards evolve. The time series will be displayed the same way
as the spectra, just that there is a time axis instead of wavelength axis, so this manual 
chapters about displaying spectra should also be considered when displaying time series.
In this case the Starlink \xref{AST}{sun211}{} library time coordinate systems are used.
...
successful then a window looking like this should appear as before:
...
case when there are many plots available. If you have so many plots open, that
the main window can not be found, just click the  \inline{splatlogo}  button in any plot window 
to bring \SPLAT's main window to front.
...
\begin{quote}
 ...
 \begin{itemize}[label={}]
  ...
  \item[] \inline{samp} Opens SAMP control window. If \SPLAT is registered to a SAMP hub,
  the icon changes to \inline{sampgo}.
...
\begin{quote}
 ...
 zero. In this case, the user is notified and asked if a copy of the plot with such values
 removed should be created). Finer control of the log spacing and labelling can be obtained 
 using the plot configuration window. If there are values 
...
\begin{quote}
 ...
 \begin{itemize}
  ...
  \item[] \inline{splatlogo} Bring the main browser window to front. This can be
  useful if there are too many open plots, and the browser window is difficult to find.
  ...
  \item[] \inline{stacker} Show the toolbox for applying offsets to the
  displayed spectra so they can be viewed more easily or shown in some order,
  for instance time of observation.
  
   \item[] \inline{speclines} Shows the  \SPLAT\ Line Browser window, which allows to search for 
  spectral lines using the VO (SLAP)  and VAMDC, and display chosen lines into the spectrum.
...
\item \subsubmenuitem{Options}{Line identifiers}{Line Browser}
  This option opens the  \SPLAT\ Line Browser window, which allows to search for 
  spectral lines using the VO (SLAP)  and VAMDC.
...
When right-clicking on a line from the query results, a popup menu will
appear with following options:
\begin{itemize}
\item Datalink - It opens the datalink kinks menu for this result. This item is
	only available if the spectra contains DataLink information. See 
\item Download - Downloads the spectrum into \SPLAT (without displaying it).
\item Info - shows preview (if available) and information on all columns returned.
\item Display - displays the the spectrum in \SPLAT.
\item Selection - show options for copying data to clipboard and pasting somewhere else.
\end{itemize}


\subheading{Implementation status}

SSAP, OBSCore, DataLink, SLAP are  being developed by the IVOA
...
data values.
Currently spectra returned from SSAP servers are mainly FITS and VOTables (partially some SED data model serialisation).  \SPLAT\ handles also VOTables using DataLink and SODA standards.  Besides that, \SPLAT\ tries to recognise and display Timeseries. This function is a prototype and will be further developed following the development of the time domain standards by IVOA.
...
The simple search interface uses the same basic parameters as the SSAP  search, with the same
functionality. Here you can select the data type for your query, wither spectrum or timeseries.
...
by editing directly the  the text area. For example, you can exchange 'spectrum' through 'timeseries' 
 to retrieve time series Please refer to the ObsCore standard for the available parameters.
(http://www.ivoa.net/documents/ObsCore/). Although this window allows all types of ObsTap queries, \SPLAT\ can handle and
display only those results from queries with \verb|dataproduct_type='spectrum'| or \verb|dataproduct_type='timeseries'|.
...
\subheading {Query results}
The ObsCore  query results interface works in the same way as described in  the  \htmlref{query results}{query_results} part of  the  SSAP window section.
 \latex{(\S\ref{query_results})}. 
...
\newpage
\subsection{DataLink - more information about the data\xlabel{datalink}}

\SPLAT\ supports \textbf{DataLink} and \textbf{SODA} (server-side manipulation through \textbf{DataLink}).
SSAP and ObsCore services providing  \textbf{DataLink} allows the user to access other resources available about the spectrum.
If the service offers \textbf{Datalink}, by right-cliking on a Spectrum from either an ObsCore or SSAP query result list, there should be an option "DataLink" in the pop-up menu.
When choosing this option, a new window will open with a list of links to resources about this data. This resources could be links to related publications, 
a preview image, or the dataset in other formats, or link to functions to perform server-side operations ( \textbf{SODA}) on the data (like cutouts, normalisation, etc).
If there's a link to a preview image then SPLAT will automatically display it on this window. If there are  \textbf{SODA}  services provided, the interactive formular to  deal with the data is also displayed.

\subheading{Server-side manipulation with DataLink/SODA}

A query response table may contain descriptions of  \textbf{SODA}  services, which can be either defined specifically for single spectra, or for all of the returned spectra. In both cases an input formular will be produced by \SPLAT\, where parameters can be entered. In the first case, the formular appears in the \textbf{Datalink} resources list when right-clicking one line from the query results list.  In the second case the soda operation can be submitted one or more selected spectra at once. This is done by clicking the Datalink button on the lower right side of the results table:
 After a SSAP or ObsCore query is sent, \SPLAT\ checks the responding servers for  \textbf{SODA} definitions. \\Table-wide  \textbf{SODA} operations are present, service will be marked with  a symbol (\inline{cutter}). If  at least one \textbf{DataLink} supporting server has been found, a \textbf{DataLink} button will appear below the query results. By clicking on it, \textbf{DataLink}. will be activated, and all results tabs will be deactivated except the results from servers that support \textbf{DataLink}. When you choose one of the active services, a window will appear, where the respective parameters for sever-side manipulation can be entered. The parameters will be set by clicking on the \textbf{set parameters} button. While  \textbf{dataLink} is activated and the parameters are set, the spectra you select and display or download will be manipulated using the chosen parameters. To deactivate \textbf{dataLink} and have retrieve the original spectra just click on the \textbf{dataLink} button again.\\

\mainfigure{datalinkparams}


\subheading{The DataLink links window}


The \textbf{DataLink}  links window is created when the Datalink option at the popup menu obtained by right-clicking on a spectrum from either an ObsCore or SSAP query result list. It contains a table containing all the links this available for the spectrum. 
The table has one row for each link, and columns containing among other information the location, content-type, size, and semantics (what is in this link).  \\
In the semantics column, \textbf{\#this} means the actual spectrum, which would be displayed by \SPLAT when clicking on the spectrum in the query results list. If a 
\textbf{\#preview} is available, it will be automatically displayed in the DataLink window. If there are \textbf{SODA} links (semantics \textbf{\#proc}), the respective form will also be displayed below the links table. The presence of an error message means link is not available. \\
As many of the links can contain objects in data formats \SPLAT  cannot understand (and are not necessarily spectra or timeseries), \SPLAT splat does not open all available links. If the content type is something that \SPLAT could understand, it tries to open as a spectrum. If the content type is \textit{text/html}, the link will be opened in a web browser. The description column should tell if it makes sense to click on the link to open it in \SPLAT or not. By right-clicking on a link, it's possible to 
copy the row, or just the access url to the clipboard, which can be used to open it with other application. 

\mainfigure{datalinklinks}


\newpage
\subsection{Spectral lines query\xlabel{lines_browser}}

To start the spectral lines browser window, click on the \inline{speclines} symbol from
a plot.
In this window, spectral lines  can be searched using SLAP and VAMDC services
and displayed into a plot. First of all, one or more wavelength ranges have to
be added to the ranges list by clicking on the {Add} Button and selecting a region
in the plot. The selected region will be marked by a gray rectangle. After the query, 
or when it's not needed anymore, it can be removed from  the plot using the {Delete} button.
To search use this window with a different plot, use the plot selection box.


\mainfigure{spectrallineswindow}


The query options that can be added are element symbol and stage/ion charge. Most SLAP 
services do not support (yet) the later, but it's supported by VAMDC services.

The selection of the services to be queried is similar to  SSAP and Obscore browsers intrfaces.
In this case, there are two services tabs, one for SLAP services, and other for VAMDC services.

The \textbf{Search} Button sends the query to the selected services. The results will appear on the
results area on the right side of the window. To display selected lines on the plot use the
use the buttons on the lower right side, or right-click on the line you want to display.
The \textbf{Remove lines} button removes all lines from the plot.

The lines browser window is linked to the plot which started it. Wavelength range selection 
can be done only in this plot, as well as spectral lines can be only displayed in this plot.

SLAP (Simple Line Access Protocol) is   being developed by the IVOA
(\htmladdnormallinkfoot{International Virtual Observatory Alliance}
{http://www.ivoa.net/}), and VAMDC services are provided by the 
(\htmladdnormallinkfoot{VAMDC Consortium}{http://www.vamdc.org/}).
...
possibilities are exhausted. Spectra retrieved by the VO often do not have an extension, 
and \SPLAT\ tries to discover their types by checking the mime type returned.
...
You can also send the results of a SSA or ObsCore query as a table from the
\labelitem{Query VO for spectra} window.  Again, use the
\menuitem{Broadcast} and \menuitem{Send} options from the
\menuitem{Interop} menu to do this. If another tool sends a table containing query results,
\SPLAT\ will receive and open it in the \labelitem{Query VO for spectra} window. 
...
\item \hitext{table.load.votable}, \hitext{table.load.fits}:
      send and receive SSA query results, or spectra
\end{itemize}

\subheading{Display options}

The \menuitem{Interop} menu contains some options that will affect the way spectra coming
from SAMP will be displayed in \SPLAT:
\begin{itemize}
 \item Same window for SAMP Spectra - When checked, all incoming SAMP spectra will be displayed in the same plot window.
  \begin{itemize}
  \item overwrite spectra - overwrite the existing spectra in the plot window (only the latest SAMP spectrum will be displayed).
  \end{itemize}
\item Handle table.load.votable as spectra - Set whether the  incoming data of this type should be handled as spectrum or list of spectra. 
\item Open SAMP coords in SSAP window - If a client sends coordinates via SAMP, add the coordinates to and open the SSAP browser window to start a search for objects at this position.
\item Open SAMP coords in ObsCore window - If another client sends coordinates via SAMP, add the coordinates to and open the ObsCore browser window to start a search for objects at this position.
...
\htmladdnormallink{Zentrum f\"ur Astronomie Heidelberg (ZAH)} {http://www.zah.uni-heidelberg.de}.
GAVO was funded through the Verbundforschung of the German Ministry for Research (BMBF) until June 2017.
Further funding for the development at  the ZAH was provided by the EU Project ASTERICS (Horizon 2020 grant agreement number 653477).  
Currently, development is funded through the EOSC infrastructure project ESCAPE (Horizon 2020 grant agreement number 824064).
