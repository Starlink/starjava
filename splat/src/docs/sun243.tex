...
SSAP and ObsCore services providing DataLink allows the user to access other resources available about the data.
By right-cliking on a Spectrum from either an ObsCore or SSAP query result list, there should be an option "DataLink" in the pop-up menu.
...
a preview image, or the dataset in other formats, or link to functions to perform server-side operations on the data (like cutouts, normalisation, etc).
If there's a link to a preview image then SPLAT will automatically display it on this window. If there are SODA (server-side operations) services provided, the interactive formular to  deal with the data is also displayed.
