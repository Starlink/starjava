...
This window allows you to inspect, set or convert the units of the data
values of your spectra.

Currently the only use made of these values in \SPLAT\ is when displaying more
than one spectrum in a plot window. In that case you can request that the data
values of all the spectra are transformed into the same system, so you can
compare values in Janskys with those in other flux per unit frequency and per
unit wavelength systems. For instance all the following systems are understood
as fluxes and can be intercompared:
\begin{quote}
\begin{tabular}{ll}
\verb+Jy+                  & ($Jansky$)                                \\
