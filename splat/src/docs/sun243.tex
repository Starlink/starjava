...
lists of the spectra that they hold about a circular region on the sky or
about a particular object. You can then select which of these you are
...
the wavelength and flux calibration options. These are located in the 
...
%mcneves
\subsubsection{Query SSAP servers for metadata parameters\xlabel{ssap_metadata}}

By selecting the \labelitem{More Parameters} button you can query the selected
SSA servers for all metadata parameters they support. The result is a table which
columns show a checkbox to select the parameter, the number of servers supporting it,
the parameter name and a value field, where you can insert a value. By putting the
mouse on a parameter name, its description will be displayed. If provided, the
default values will be inserted to the value field. the parameter. The checked
parameters having a non empty value  will be added to the query.

\mainfigure{ssapMetadataTable}

After selecting the desired parameters and adding values, they will be included to the query when
the \labelitem{Go} button is selected on the Query VO  for Spectra window.  
The current list of parameters can be written to, or read from a file, as well as updated from the
current list of SSA servers.
%mcneves
\newpage
