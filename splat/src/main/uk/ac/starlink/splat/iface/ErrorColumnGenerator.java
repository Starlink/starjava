    ...
    extends ColumnGenerator
{
    ...
    protected static String[][] templates = {
        { "Poisson errors", "sqrt(data)/gain", 
                            "gain", "1.0" },
        { "Sky subtracted errors", "sqrt((data+meansky)/gain)", 
                                   "meansky", "0.0",
                                   "gain", "1.0" },
        { "Gaussian errors", "const", 
                             "const", "100.0" },
        { "Linear transform of data", "data*scale+zero", 
                                      "scale", "1.0", 
                                      "zero", "0.0" },
        { "Linear transform of errors", "error*scale+zero", 
                                        "scale", "1.0", 
                                        "zero", "0.0" },
        { "Poisson distribution", "sqrt(poisson(mean))", 
                                  "mean", "10.0" },
        { "Poisson distribution added to data", "data+poisson(mean)", 
                                                "mean", "10.0" },
        { "Gaussian distribution", "gauss(mean,sigma)", 
                                   "mean", "10.0", 
                                   "sigma", "1.0" },
        { "Gaussian distribution added to data", "data+gauss(mean,sigma)", 
                                                 "mean", "10.0",  
                                                 "sigma", "1.0" },
        { "Random distribution", "rand(lower,upper)", 
                                 "lower", "0.0", 
                                 "upper", "1.0" },
        { "Log of error (natural)", "error/data" },
        { "Log of error (base a)", "error/(data*log(a))", 
                                   "a", "10" },
        { "Envelope function", "error*exp(-0.5*(radius/sigma)**2)", 
                               "radius", "coord-center",
                               "center", "100.0",
                               "sigma", "50.0" },
    };
