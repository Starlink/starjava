...
/**
 * HdxInputNameParser - temporary class for encapsulating naming
 * strategies that differentiate the various data storage formats.
 * <p>
 * These names may be simple disk file names, such as the top-level
 * containers of NDF and data, or more complex ones that
 * include NDF slices, FITS extensions and possibly HDS component
 * paths. XML containers are everything else!
 * <p>
 ...
 *   <li> file.sdf -- an NDF.
 *
 *   <li> file.sdf(1:100,2:200) -- NDF with slice.
 *
 *   <li> file.ndf -- NDF stored in a container file, expanded to
 *                    file.sdf.ndf.
 *
 ...
 *                                     expanded.
 *  </ul>
 *  To use this class create an object with the specification, then
 *  use its methods to get fully qualified names, slices, paths and
 *  disk file names.
 *  <p>
 *  TODO: will need to use something in HDX to do this!
 *
...
public class HdxInputNameParser
{
     ...
     *  Disk file type.
     */
    private String type_ = ".sdf";

    /**
     *  Constructor - single argument, the specification.
     */
    public HdxInputNameParser( String specification )
    {
     ...
     *  Set the name.
     */
    public void setName( String specification )
    {
        this.specification = specification;
        parseName();
    }
     ...
     *  "FITS" or "XML", depending on what the file extension and
    ...
    public String format()
    {
        ...
        return "UNKNOWN"; //  Overload of anything is XML with HDS path...
    }
    ...
    public String ndfname()
    {
        ...
        //  Foreign format (TODO: is this needed?). Always return
        //  these in single quotes to protect special characters.
        if ( fitshdu_ != 0 ) {
    ...
    public String absolute()
    {
        ...
        specification = file.getAbsolutePath();
        parseName();
    ...
    {
        int i1 = specification.lastIndexOf( "(" );
        int i2 = specification.lastIndexOf( ")" );
        if ( i1 > -1 && i2 > -1 ) {
            slice_ = specification.substring( i1, i2 + 1 );
        } else {
            slice_ = "";
        }
    }
    ...
    protected void getFitsext()
    {
        ...
        int i2 = specification.lastIndexOf( "]" );
        if ( i1 > -1 && i2 > -1 ) {
            fitsext_ = specification.substring( i1, i2 + 1 );
            try {
                fitshdu_ = Integer.parseInt(
                    specification.substring( i1 + 1, i2 ) );
            } catch ( Exception e ) {
                fitshdu_ = 0;
            }
        } else {
    ...
    /**
     *  Check if the file "type" is known.
     */
    ...
    {
        int i1 = specification.indexOf( type_ );
        if ( i1 > -1 ) {
            diskfile_ = specification.substring( 0, i1 ) + type_;
        } else {

            //  Type not in specification, so fallback to path_.
            i1 = specification.indexOf( path_ );
            if ( i1 > -1 && ! path_.equals( "" ) ) {
                ...
                int i2;
                if ( ! slice_.equals( "" ) ) {
                    i2 = specification.indexOf( slice_ );
                    diskfile_ = specification.substring( 0, i2 ) + type_;
                } else {
                    diskfile_ = specification + type_;
                }
            }
        }
    }
    ...
    {
        HdxInputNameParser namer = new HdxInputNameParser();
        String names[] = {
            "container.xml",
            "unknown.type",
            "file.fits",
