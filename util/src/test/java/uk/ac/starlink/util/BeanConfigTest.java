...
public class BeanConfigTest {
    ...
    public void testTBean() throws ReflectiveOperationException, LoadException {
        ...
        assertNull( tb0.primary_ );
        assertNull( tb0.text_ );

        TBean tb1 = createTBean( "(ival=23,primary= BLUE, text=elder thing)" );
        assertEquals( 23, tb1.ival_ );
        Assertions.assertEquals(Primary.BLUE, tb1.primary_);
        Assertions.assertEquals("elder thing", tb1.text_);

        assertEquals( 42, createTBean( "(ival=0x2a)" ).ival_ );
        Assertions.assertEquals(Primary.GREEN, createTBean("(ival=09,primary=Green)").primary_);
        assertNull( createTBean( "(primary=)" ).primary_ );

        Assertions.assertEquals(Dir.LEFT, createTBean("( dir = LEFT )").dir_);
        Assertions.assertEquals(TBean.NORTH, createTBean("(dir=NORTH)").dir_);
        Assertions.assertEquals(TBean.NORTH, createTBean("(dir=North)").dir_);

        Assertions.assertEquals(Dir.LEFT, createTBean("(dir=links)").dir_);
        Assertions.assertEquals(Dir.RIGHT, createTBean("(dir=droite)").dir_);
        try {
        ...
        assertNull( createTBean( "(dir=)" ).dir_ );
