/*
 * $Id: BasicManhattanRouter.java,v 1.12 2000/08/16 20:31:03 neuendor Exp $
 *
...
public class BasicManhattanRouter implements ManhattanRouter {
	...
	Polyline2D route = _route(headPt, headDir,
				  tailPt, tailDir);                        
	return route;
	...
	*/
    ...
    private Polyline2D _route(Point2D head, int headDir, 
            Point2D tail, int tailDir) {        
	...
	if(headDir == SwingUtilities.WEST) {
		...
		if(xDiff < 0) {
		    ...
		    point = new Point2D.Double(tail.getX(), 
                            head.getY());
		} else if(headDir == tailDir) {
		    double pos = 
			Math.min(head.getX(), tail.getX()) - MINDIST;
		...
		if(xDiff > 0) {
		    ...
		    double pos = 
			Math.max(head.getX(), tail.getX()) + MINDIST;
		    point = new Point2D.Double(pos, head.getY());
		} else {
		    point = new Point2D.Double(head.getX() - xDiff/2, 
					       head.getY());
		...
		if(yDiff > 0) {
		    ...
		    double pos = 
			Math.max(head.getY(), tail.getY()) + MINDIST;
		...
		if(yDiff < 0) {
		    ...
		    double pos = 
			Math.min(head.getY(), tail.getY()) - MINDIST;
		    point = new Point2D.Double(head.getX(), pos);
		} else {
		    point = new Point2D.Double(head.getX(), 
					       head.getY() - yDiff / 2);
