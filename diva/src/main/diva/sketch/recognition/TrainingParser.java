/*
 * $Id: TrainingParser.java,v 1.9 2000/08/04 01:24:02 michaels Exp $
 *
...
/**
 * TrainingParser reads in an XML file and parses it into a
 * TrainingModel data structure.  The XML file should conform to
 * train.dtd format so that it can be parsed correctly.
 *
...
public class TrainingParser extends HandlerBase implements ModelParser {
    ...
    "<!ELEMENT trainingModel (type+)> <!ELEMENT type (example+)> <!ATTLIST type name CDATA #REQUIRED> <!ELEMENT example (timedStroke)> <!ATTLIST example label CDATA #REQUIRED points CDATA #REQUIRED>";
     ...
     */
    public TrainingModel parse(Reader[] readers) throws java.lang.Exception  {
        TrainingModel models[] = new TrainingModel[readers.length];
        for(int i=0; i< readers.length; i++){
            models[i] = (TrainingModel)parse(readers[i]);
        }
        TrainingModel combinedModel = new TrainingModel();
