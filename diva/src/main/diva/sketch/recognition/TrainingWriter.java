/*
 * $Id: TrainingWriter.java,v 1.5 2000/07/04 18:23:44 hwawen Exp $
 *
...
/**
 * TrainingWriter takes a training model and writes it out to an
 * outputstream.
 *
 * @author Heloise Hse      (hwawen@eecs.berkeley.edu)
 * @version $Revision: 1.5 $
 */
public class TrainingWriter implements ModelWriter {
    /**
    ...
    public void writeModel(Object m, Writer writer) throws IOException{
        ...
        writeHeader(writer);
        writer.write("<"+TrainingParser.MODEL_TAG+">\n");
        for(Iterator i = model.types(); i.hasNext();){
            String type = (String)i.next();
            writer.write("<" + TrainingParser.TYPE_TAG + " " + TrainingParser.NAME_TAG + "=\"");
            writer.write(type);
            writer.write("\">\n");
            if(model.positiveExampleCount(type)>0){
                for(Iterator pi = model.positiveExamples(type); pi.hasNext();){
                    ...
                    writeExample(stroke, TrainingModel.POSITIVE, writer);
                }
            }
            if(model.negativeExampleCount(type)>0){
                for(Iterator ni = model.negativeExamples(type); ni.hasNext();){
                    ...
                    writeExample(stroke, TrainingModel.NEGATIVE, writer);
                }
            }
            writer.write("</" + TrainingParser.TYPE_TAG + ">\n");
        }
        writer.write("</" + TrainingParser.MODEL_TAG + ">\n");
    }
    ...
    private void writeExample(TimedStroke stroke, boolean label, Writer writer)
            throws IOException {
        String lbl = (label) ? "+" : "-";
        writer.write("<" + TrainingParser.EXAMPLE_TAG + " " + TrainingParser.LABEL_TAG + "=\"");
        writer.write(lbl);
    ...
    private void writeHeader(Writer writer) throws IOException {
        ...
        writer.write("<!DOCTYPE " + TrainingParser.MODEL_TAG + " PUBLIC \""
                + TrainingParser.PUBLIC_ID + "\"\n\t\""
                + TrainingParser.DTD_URL + "\">\n\n");
    }
}
