/*
 * $Id: SineInitAngleFE.java,v 1.6 2000/05/10 18:54:54 hwawen Exp $
 *
...
public class SineInitAngleFE implements FeatureExtractor {
    ...
    public double apply(TimedStroke s) {
        if(s.getVertexCount() >= 3) {
            double denom = FEUtilities.distance(s.getX(0), s.getY(0), s.getX(2), s.getY(2));
            if(denom==0){
            ...
            double denom = FEUtilities.distance(s.getX(0), s.getY(0), s.getX(1), s.getY(1));
            if(denom==0){
