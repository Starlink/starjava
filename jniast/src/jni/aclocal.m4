...
# messgen package.
#
# This macro also declares that the .msg files are pre-distribution
# files (it calls STAR_PREDIST_SOURECS) on them, and so the resulting
# configure script should expect not to find them in an unpacked
# distribution.
#
# This macro leaves the discovered facility code in the shell variable
# star_facilitycode (this corresponds to the last message file, if there
# is more than one), from where it may be scavenged by the calling
# configure script.  No component should _need_ to use this number -- it
# is present for historical reasons, and is potentially useful if a
# component needs an integer which is guaranteed unique to that component.
#
# Note that this macro determines the facility code when configure is
# being generated (as opposed to when it is being run on the build
# machine), since the .msg file will typically not be distributed, and
# thus not available when ./configure is run)
         ...
         [#
          ...
          AC_FOREACH([MsgFileName], [$1],
                     [m4_define([MsgFacCode],
                      ...
                      STAR_HELP_FILES([fac_]MsgFacCode[_err])dnl
                      _STAR_EXTRA_DIST([fac_]MsgFacCode[_err])dnl
                      star_facilitycode=MsgFacCode
                     ])
          STAR_DECLARE_DEPENDENCIES([sourceset], [messgen])
          STAR_PREDIST_SOURCES($1)
          ...
          m4_ifval([$2],
                   ...
                   [dnl second arg empty -- use defaults
                    if $_star_build_docs; then
                        for d in $1
                        do
                            _STAR_EXTRA_DIST($d.tex $d.ps $d.htx_tar)
                            STAR_DOCS_FILES($d.tex $d.ps $d.htx_tar)
                        done
                    else
                        AC_MSG_WARN([not installing docs $1])
                    fi
...
# `sourceset', `build', `link', `use' or `test', and the DEPLIST is a space
...
#
# The variables starXdir (for X in the list above) are defined in
# STAR_DEFAULTS, and the prefixes starX are added to the legal 'DATA'
# prefixes in automake:handle_data.
AC_DEFUN([_STAR_EXTRADIR_COMMON],
         [m4_ifval([$1], [], [AC_FATAL([$0: must have two args])])dnl
          m4_ifval([$2], [], [AC_FATAL([$0: must have two args])])dnl
          m4_if(m4_bregexp([$1],
                           [^\(docs\|etc\|help\|examples\)$]),
                [0],
                [],
                [AC_FATAL([$0: unrecognised type $1])])dnl
          AC_SUBST(star$1_DATA)dnl
          star$1_DATA="[$]star$1_DATA $2"
...
# the distribution.  This has the same overall function as automake's
# EXTRA_DIST rule, except that this is under the control of configure.
