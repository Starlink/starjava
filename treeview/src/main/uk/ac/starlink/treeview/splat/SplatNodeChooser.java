 ...
 *       Original version, based on Mark's NdxNodeChooser & TableNodeChooser.
 */
package uk.ac.starlink.treeview.splat;
...
import uk.ac.starlink.treeview.CreationState;
import uk.ac.starlink.treeview.DataNode;
import uk.ac.starlink.treeview.DataNodeFactory;
import uk.ac.starlink.treeview.FITSFileDataNode;
import uk.ac.starlink.treeview.FileDataNode;
import uk.ac.starlink.treeview.NdxNodeChooser;
import uk.ac.starlink.treeview.PlainDataNode;
import uk.ac.starlink.treeview.TableNodeChooser;
import uk.ac.starlink.treeview.TreeNodeChooser;
import uk.ac.starlink.treeview.TreeviewUtil;
import uk.ac.starlink.treeview.XMLDataNode;
    ...
    extends TreeNodeChooser
{
    ...
    protected boolean isChoosable( DataNode node )
    {
        ...
        if ( node instanceof TableNodeChooser.Choosable ) {
            if ( ((TableNodeChooser.Choosable) node).isStarTable() ) {
                return true;
            }
        }
        ...
        if ( node instanceof NdxNodeChooser.Choosable ) {
            if ( ((NdxNodeChooser.Choosable) node).isNdx() ) {
                return true;
            }
        }
        return false;
     ...
     *
     * @param node a {@link DataNode} that is guaranteed to be an instance of 
     *             {@link TableNodeChooser.Choosable}.
     */
    protected SpecData makeSpecDataFromTable( DataNode node )
        throws IOException, SplatException
    {
        StarTable table = ((TableNodeChooser.Choosable) node).getStarTable();

        // Generate suitable short and full names for this table.
        String fullName = TreeviewUtil.getNodePath( node );
    ...
    /**
     * Make a {@link SpecData} object from an {@link NdxNodeChooser.Choosable}
     * that contains an {@link Ndx}.
     *
     * @param node a {@link DataNode} that is guaranteed to be an instance of 
     *             {@link NdxNodeChooser.Choosable}.
     */
    protected SpecData makeSpecDataFromNdx( DataNode node )
        throws IOException, SplatException
    {
        Ndx ndx = ((NdxNodeChooser.Choosable) node).getNdx();
